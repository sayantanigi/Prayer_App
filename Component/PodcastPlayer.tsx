...
import { Animated, Dimensions, View, Text, StyleSheet, Pressable, Image, BackHandler } from "react-native";
import { useMusicStore } from "../store/music";
import Slider from "@react-native-community/slider"
import { BackgroundColor, Black, PrimaryColor, TextColor } from "../Constant/Color";
import type { PodcastPlayer } from "../store/music";
import { useSwipe, useSwipeHorizontal } from "../hooks/onSwipe";
import { Icon_31, Icon_32, Icon_33, Icon_34, Icon_35, Icon_36, Icon_37 } from "../Constant/Images";
...
function TopView({ audioState, playerState, toggleAudio, onTouchEnd, onTouchStart }: TopView) {
    const [props] = useMusicStore();
    const width = React.useRef(new Animated.Value(windowWidth - 40, { useNativeDriver: false })).current;
        ...
        <View style={[styles.Topbar]}>
            <View
                ...
                <Animated.View style={{ opacity, paddingLeft: 10, width: "60%" }}>
                    <Text style={styles.MinArtist} numberOfLines={1}>{props?.artist}</Text>
                    <Text style={styles.MinTitle} numberOfLines={1}>{props?.title}</Text>
                    <Text style={styles.MinDetails} numberOfLines={1}>{props?.details}</Text>
                </Animated.View>
...
export default function PodcastPlayer() {
    ...
    const { onTouchStart: onTouchStartRight, onTouchEnd: onTouchEndRight } = useSwipeHorizontal(handleRightSwipe)
    const [props, setProps] = useMusicStore();
    const [playerState, setPlayerState] = React.useState<PlayerState>(PlayerState.Maximized)
    const [sound, setSound] = React.useState<Audio.Sound>();
    const [audioState, setAudioState] = React.useState<AudioState>(AudioState.Paused)
    const [position, setPosition] = React.useState(0);
    const [duration, setDuration] = React.useState(1);
    const range = React.useMemo(() => position / duration * 100, [position, duration])
    const height = React.useRef(new Animated.Value(windowHeight, { useNativeDriver: false })).current;
    const translateX = React.useRef(new Animated.Value(0, { useNativeDriver: false })).current;
    ...
    return (
        <Animated.View
            style={[styles.PlayerContainer, { height, width, bottom, left, borderRadius, padding, opacity: boxOpacity, transform: [{ translateX }] }]}>
            <Pressable
                ...
                <Animated.View style={[styles.RangeBox, { opacity }]}>
                    <Text numberOfLines={1} style={styles.PodcastArtist}>{props?.artist}</Text>
                    <Text numberOfLines={2} style={styles.PodcastTitle}>{props?.title}</Text>
                    <Text numberOfLines={1} style={styles.PodcastDetails}>{props?.details}</Text>
                    <View style={styles.TimeBox}>
                        <Text style={styles.TimeText}>{new Date(position).toISOString().slice(11, 19)}</Text>
                        <Text style={styles.TimeText}>{new Date(duration).toISOString().slice(11, 19)}</Text>
                    </View>
                    <Slider
                    ...
                    <View style={styles.ActionBox}>
                        {likeShow ? (
                            <>
                                <Pressable onPress={() => setLikeShow(!likeShow)}>
                                    <Image
