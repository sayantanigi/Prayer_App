...
export default function O_PrayerAddScreen() {
  ...
  const BannerImage = React.useMemo(() => {
    if (files.length > 0) {
        setMainfile({ type: files[0].mimeType as string, name: files[0].name, uri: files[0].uri })
  ...
  const handleConfirm = (date: any) => {
   ...
   // const formattedDate = moment(date).format('YYYY-MM-DD');
    ...
    setForm({ ...form, prayer_datetime: formattedDate })
  ...
  const SuccessAlert = async () => {
    ...
    formData.append('prayer_datetime', form.prayer_datetime)
    formData.append('prayer_description', form.prayer_description)
    formData.append('prayer_location', "")
      ...
      <View style={styles.Container}>
        ...
        <View style={styles.InputBlock}>
          ...
          <View style={styles.Input}>
            <TextInput
              style={styles.InputField}
              onChangeText={(text) => handleValueChange(text, "prayer_subheading")}
          ...
          <TouchableOpacity  style={styles.InputField} onPress={showDatePicker}>
            <TextInput
             editable={false}
              ...
              style={styles.InputField}
              value={inputValue}
              //onChangeText={(value) => handleValueChange(value, "prayer_datetime")}
        ...
        <View style={styles.InputBlock}>
          ...
          <View style={[styles.Input, styles.CommentInput]}>
            <TextInput
              ...
              multiline={true}
              onChangeText={(text) => handleValueChange(text, "prayer_description")}
