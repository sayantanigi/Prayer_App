...
import { View, StyleSheet, Text, SafeAreaView, Pressable, Image, Dimensions, Animated } from "react-native";
import { Calendar } from 'react-native-calendars';
import EventTimeAccordion from "../../../../Component/EventTimeAccordion";
import { Black, PrimaryColor, StatusBarBack, TextColor, White } from "../../../../Constant/Color";
import U_ScreenLayout from "../U_ScreenLayout";
import { Back_1, BlankImage, HomeIcon_2 } from "../../../../Constant/Images";
...
export function getWeekList(date: Date = new Date()) {
    ...
    let dates = [];
    let firstDateofWeek = new Date(date).setDate(new Date(date).getDate() - todayWeekDay);
    for (let i = 0; i < 7; i++) {
...
export default function U_EventScreen() {
    ...
    const weekInfo = React.useMemo(() => {
        ...
        let weekData = `${firstWeekDate.getDate()} - ${lastWeekDate.getDate()} ${lastWeekDate.toLocaleDateString("en", { month: "long" })}`
        return weekData
    }, [date])
    const translateX = React.useRef(new Animated.Value(calculateTranslateX(active))).current;
        ...
        >
            <SafeAreaView style={styles.Container}>
                <View style={styles.EventHeaderContainer}>
                    <Text style={styles.EventHeaderText}>Calendar of Prayers in Prayer App</Text>
                </View>

                {/* EventTimeTable */}
                <EventTimeAccordion weekInfo={weekInfo}>
                    <Calendar style={{ width: "100%", paddingBottom: 15, }}
                        onDayPress={date => {
                            setDate(date.dateString)
                        }}
                        markedDates={{
                            [date]: { selected: true, selectedColor: "#741623", selectedTextColor: "#ffffff" }
                        }}
                    />
                </EventTimeAccordion>
                ...
                <View style={styles.EventWeekCalender}>
                    <Animated.View style={[styles.weekIndicator, { transform: [{ translateX }] }]}>
                    </Animated.View>
                    <View style={styles.WeekBox}>
                        {week.map((date, index) => {
                            if (active !== index) {
                                return (
                                    <Pressable onPress={() => setDate(getCurrentDate(date))} style={styles.WeekDate} key={index}>
                                        <Text style={styles.DayName}>{(WeekDays as any)[date.getDay()]}</Text>
                                ...
                                return (
                                    <Pressable onPress={() => setDate(getCurrentDate(date))} style={styles.WeekDate} key={index}>
                                        <Text style={styles.DayNameActive}>{(WeekDays as any)[date.getDay()]}</Text>
                ...
                <Animated.View style={[styles.EventBoxWrap, { transform: [{ translateX: eventBoxX }] }]}>
